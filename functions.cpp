...
vector<vector<double>> multiquadratic_result_extrapolate(results_t results) {
	...
	vector<double> ys;
	for (auto result : results) {
		if (result.second[LABEL_INDEX] == 0.0) {
...
// Reads a configuration file at filename, returns true iff config was not
// read successfully
bool read_config(string filename, config_t& config) {
	...
	try {
		...
		string is_discrete_string = data.at(3).at(0);
		...
		config.is_discrete = data.at(3).at(0).compare("true") == 0;
		config.constraints = stoul(data.at(4).at(0));
		config.costs = stoul(data.at(5).at(0));
		config.boundaries = read_boundaries(data.at(6), data.at(7));
		if (!are_valid_boundaries(config.boundaries)) {
			throw invalid_argument("invalid boundaries");
		}
		config.sig_level = stof(data.at(8).at(0));
		if (config.sig_level < 0.0 || config.sig_level > 1.0) {
			throw invalid_argument("invalid significance level");
		}
		config.fitness_percentile = stof(data.at(9).at(0));
